...
def build_pipeline(settings) -> CSVClassificationPipeline:
    ...
    embeddings = EmbeddingService(model_path=settings.embedding_model_path)
    store = VectorStore(embeddings, settings=settings)
    if store.count() == 0:
        raise SystemExit(
            "knowledge base is empty; restore and configure the frozen collection "
            "as described in docs/RAG_FREEZE.md"
        )
    classifier = FieldClassificationService(store, LLMService(settings=settings))
